    ...
    for _, link in ipairs(dom.select(html, 'a[href*="/title/"], a[href*="/series/"], a[href*="/manga/"]')) do
        ...
        if url and title ~= "" and not seen[url] then
            ...
            results[#results + 1] = { title = title, manga_title = title, url = url, manga_url = url,
                thumbnail_url = absolute(image), type = "Manga", sourceType = "Manga", source = "MangaBall", language = "en" }
        end
    end
    return results
end
...
function M.manga_details(url)
    ...
    return { title = title, url = url, genres_json = json.encode(genres), thumbnail_url = absolute(image), type = "Manga", source = "MangaBall", language = "en" }
end
