    ...
    for _, link in ipairs(dom.select(html, 'a[href*="/manga/"], a[href*="/series/"], a[href*="/title/"]')) do
        ...
        if url and title ~= "" and not seen[url] then
            ...
            results[#results + 1] = { title = title, manga_title = title, url = url, manga_url = url,
                thumbnail_url = cover, type = kind, sourceType = kind, source = "Kagane", language = "en" }
        end
    end
    return results
end
...
function M.manga_details(url)
    ...
    return { title = title, url = url, genres_json = "[]", thumbnail_url = absolute(image), type = kind, source = "Kagane", language = "en" }
end
